...
public class UpdateCheck extends JFrame {
    ...
    public UpdateCheck() {
        // Sample database connection (make sure to replace with your own connection setup)
        try {
            conn = DriverManager.getConnection("jdbc:mysql:///hotelmanagement", "root", "Naveen@123");
        ...
        JLabel lblUpdateCheckStatus = new JLabel("Update Details");
        ...
        // Adding an image (make sure the path is correct)
        ...
        btnUpdate.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                try {
                    ...
                    conn.createStatement().executeUpdate("UPDATE customer SET room = '" + s2 + "', name = '" + s3 + "', status = '" + s4 + "', deposit = '" + s5 + "' WHERE number = '" + s1 + "'");
        ...
        btnAdd.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    String s1 = c1.getSelectedItem();
                    ResultSet rs1 = conn.createStatement().executeQuery("SELECT * FROM customer WHERE number = '" + s1 + "'");
                ...
                try {
                    String total = "";
                    ResultSet rs2 = conn.createStatement().executeQuery("SELECT * FROM room WHERE room_number = '" + txt_ID.getText() + "'");
                    while (rs2.next()) {
